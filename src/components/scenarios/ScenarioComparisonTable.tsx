...
export default function ScenarioComparisonTable({ scenarios, currency, businessType }: ScenarioComparisonTableProps) {
  ...
  if (businessType === 'saas') {
    metrics.push(
      {
        label: 'MRR',
        optimistic: optimisticData?.mrr || 0,
        realistic: realisticData?.mrr || 0,
        pessimistic: pessimisticData?.mrr || 0,
        format: formatCurrency,
      },
      {
        label: 'ARR',
        optimistic: optimisticData?.arr || 0,
        realistic: realisticData?.arr || 0,
        pessimistic: pessimisticData?.arr || 0,
        format: formatCurrency,
      },
      {
        label: 'LTV',
        optimistic: optimisticData?.ltv || 0,
        realistic: realisticData?.ltv || 0,
        pessimistic: pessimisticData?.ltv || 0,
        format: formatCurrency,
      },
      {
        label: 'CAC',
        optimistic: optimisticData?.cac || 0,
        realistic: realisticData?.cac || 0,
        pessimistic: pessimisticData?.cac || 0,
        format: formatCurrency,
      }
    );
  }
  ...
  if (businessType === 'b2c-platform') {
    metrics.push(
      {
        label: '총 GMV',
        optimistic: optimisticData?.totalGmv || 0,
        realistic: realisticData?.totalGmv || 0,
        pessimistic: pessimisticData?.totalGmv || 0,
        format: formatCurrency,
      },
      {
        label: '총 주문수',
        optimistic: optimisticData?.totalOrders || 0,
        realistic: realisticData?.totalOrders || 0,
        pessimistic: pessimisticData?.totalOrders || 0,
        format: formatNumber,
      },
      {
        label: '평균 Take Rate',
        optimistic: optimisticData?.averageTakeRate || 0,
        realistic: realisticData?.averageTakeRate || 0,
        pessimistic: pessimisticData?.averageTakeRate || 0,
        format: formatPercent,
      },
      {
        label: '총 환불',
        optimistic: optimisticData?.totalRefunds || 0,
        realistic: realisticData?.totalRefunds || 0,
        pessimistic: pessimisticData?.totalRefunds || 0,
        format: formatCurrency,
      }
    );
  }
